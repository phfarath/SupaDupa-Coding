...
export async function providerCommand(action, name, options = {}) {
  try {
...
async function handleAdd(registry, name, options) {
  if (!name) {
...
async function handleList(registry) {
...
async function handleSwitch(registry, name) {
  if (!name) {
...
async function handleRemove(registry, name) {
  if (!name) {
...
async function handleShow(registry, name) {
  if (!name) {
  ...
  const provider = await registry.getProvider(name, false);
...
async function handleUpdate(registry, name, options) {
  if (!name) {
