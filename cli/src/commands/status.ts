...
export async function statusCommand(options: StatusCommandOptions) {
  try {
    ...
    if (options.all) {
      ...
      // Show specific feature status - using a mock implementation since the actual orchestrator getStatus functionality isn't specified
      ...
      const status: FeatureStatus = {
        ...
        tasks: [
          ...
          { name: 'implement-feature', status: 'in-progress' },
          { name: 'test-implementation', status: 'pending' }
        ]
      };
      ...
      const summary: MetricsSummary = metrics.getSummary();
