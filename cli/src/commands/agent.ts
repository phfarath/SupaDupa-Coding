...
export async function agentCommand(action: string = 'list', name?: string, options: AgentCommandOptions = {}) {
  try {
    // Load configuration
    ...
    if (config.customAgents) {
      for (const [agentName, agentConfig] of Object.entries(config.customAgents)) {
        const customAgent = createCustomAgent(agentName, agentConfig as AgentConfig);
...
function createCustomAgent(name: string, agentConfig: AgentConfig): Agent {
  return {
    name,
...
function listAgents(orchestrator: any, options: AgentCommandOptions = {}) {
  console.log('\n' + chalk.bold.cyan('Registered Agents'));
  console.log(chalk.gray('─'.repeat(50)));
  
  const agents: [string, Agent][] = Array.from(orchestrator.agents.entries());
  ...
  if (options.type) {
    filteredAgents = agents.filter(([_name, agent]) => {
      const info = agent.getInfo();
      return info.config?.type === options.type;
    });
  }
  if (options.status) {
    filteredAgents = filteredAgents.filter(([_name, agent]) => {
...
function showAgentInfo(orchestrator: any, name: string) {
...
async function createAgent(orchestrator: any, name: string | undefined, options: AgentCommandOptions, configManager: ConfigManager) {
  if (!name) {
...
async function startAgent(orchestrator: any, name: string | undefined) {
  if (!name) {
    console.error(chalk.red('Error: Agent name is required'));
...
async function stopAgent(orchestrator: any, name: string | undefined) {
  if (!name) {
    console.error(chalk.red('Error: Agent name is required'));
...
async function restartAgent(orchestrator: any, name: string | undefined) {
  if (!name) {
    console.error(chalk.red('Error: Agent name is required'));
...
async function deleteAgent(orchestrator: any, name: string | undefined, configManager: ConfigManager) {
  if (!name) {
    console.error(chalk.red('Error: Agent name is required'));
