...
export async function apiCommand(action = 'status', providerName, options = {}) {
  try {
...
async function registerProvider(providerName, options) {
  if (!providerName) {
...
async function showApiStatus(options) {
...
export async function authCommand(action = 'configure', providerName, options = {}) {
  try {
    switch (action) {
      case 'configure':
...
async function configureAuth(providerName, options) {
  if (!providerName) {
    console.error(chalk.red('Error: Provider name is required'));
...
async function verifyAuth(providerName, options) {
  if (!providerName) {
    console.error(chalk.red('Error: Provider name is required'));
