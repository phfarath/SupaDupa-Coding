...
export async function workflowCommand(action = 'list', workflowId, options = {}) {
  try {
...
async function runWorkflow(workflowId, options, config) {
  if (!workflowId) {
...
async function workflowStatus(workflowId, options) {
  ...
  try {
    ...
    const execution = JSON.parse(logData);
...
async function workflowLogs(workflowId, options) {
  if (!workflowId) {
    console.error(chalk.red('Error: Workflow ID is required'));
