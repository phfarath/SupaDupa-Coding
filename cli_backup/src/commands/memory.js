...
export async function memoryCommand(action = 'init', subAction, options = {}) {
  try {
...
async function handleContext(subAction, options) {
  switch (subAction) {
    case 'show':
...
async function showContext(options) {
...
async function backupContext(options) {
  const agentId = options.agent;
